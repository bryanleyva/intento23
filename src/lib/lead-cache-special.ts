...
export class LeadCacheSpecial {
    private static instance: LeadCacheSpecial;
    private rows: any[] = [];
    ...
    public static getInstance(): LeadCacheSpecial {
        if (!LeadCacheSpecial.instance) {
            LeadCacheSpecial.instance = new LeadCacheSpecial();
        }
        return LeadCacheSpecial.instance;
    }
    ...
    private async _refresh() {
        try {
            ...
            if (!sheet) {
                console.error('LeadCacheSpecial: Sheet BASE SPECIAL not found');
                return;
            }
            ...
            console.error('LeadCacheSpecial: Error refreshing data', error);
        }
    }
    ...
    public async updateRow(ruc: string, updates: Record<string, any>, options?: { onlyIfUnassigned?: boolean }) {
        return this.runLocked(async () => {
            ...
            try {
                ...
                console.error('LeadCacheSpecial: Error saving row', e);
                return false;
            }
        });
    }
