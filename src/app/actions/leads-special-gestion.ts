...
export async function getNextSpecialLead(userEmail: string, userName: string) {
    try {
        const cache = LeadCacheSpecial.getInstance();
        ...
        if (!result.row) {
            return { message: 'No hay registros de Base Special asignados.' };
        }

        return { success: true, data: mapRow(result.row) };
    } catch (error) {
        console.error('Error en getNextSpecialLead:', error);
        return { error: 'Error interno al procesar Base Special.' };
    }
}

export async function saveSpecialLead(rowIndex: number, data: any) {
    try {
        const cache = LeadCacheSpecial.getInstance();
        await cache.ensureInitialized();
        ...
        console.error('Error en saveSpecialLead:', error);
        return { success: false, error: 'Error al guardar.' };
    }
}

export async function getSpecialAgendamientos(userName: string, userRole: string) {
    try {
        await loadDoc();
        const sheet = doc.sheetsByTitle['BASE SPECIAL'];
        if (!sheet) return { error: 'Hoja BASE SPECIAL no encontrada' };
        ...
        console.error('Error en getSpecialAgendamientos:', error);
        return { error: 'Error al obtener agendamientos' };
    }
}

export async function getSpecialLeadByRuc(ruc: string) {
    try {
        const cache = LeadCacheSpecial.getInstance();
        await cache.ensureInitialized();
        ...
        console.error('Error en getSpecialLeadByRuc:', error);
        return { success: false, error: 'Error al cargar lead' };
    }
}

export async function createManualSpecialLead(data: any, user: string) {
    try {
        await loadDoc();
        const sheet = doc.sheetsByTitle['BASE SPECIAL'];
        if (!sheet) return { success: false, error: 'Hoja BASE SPECIAL no encontrada.' };
        ...
        console.error('Error en createManualSpecialLead:', error);
        return { success: false, error: error.message || 'Error al crear registro.' };
    }
}

export async function getSpecialAssignmentStats(userRole?: string, userName?: string) {
    try {
        const cache = LeadCacheSpecial.getInstance();
        await cache.ensureInitialized();
        ...
        console.error('Error en getSpecialAssignmentStats:', error);
        return { success: false, error: 'Error al cargar estadísticas de Base Special' };
    }
}

export async function assignSpecialLeadsByCriteria(
    ...
    try {
        ...
        const criteria = (row: any) => {
            ...
            switch (rangeId) {
                ...
                default: return true; // 'TODOS' = cualquier cantidad
            }
        };
...
export async function promoteToPipelineFromSpecial(lead: any, user: string) {
    try {
        await loadDoc();
        ...
        console.error('Error en promoteToPipelineFromSpecial:', error);
